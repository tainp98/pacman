...
class App:
    def __init__(self,select=None):
        ...
        # if self.select == 'bfs':
        #     self.bfs = BFS(self)
        # path = self.bfs.BFS(vec(2,1),vec(2,8))
        # print('path',path)
        ...
        with open("wall.txt","r") as file:
            for yidx,line in enumerate(file):
                for xinx, char in enumerate(line):
                    ...
                    # else:
                    #     self.coins.append(vec(xinx, yidx))
...
####### PLAYING FUNCTION
    ...
    def playing_draw(self):
        ...
        if self.select == 'bfs':
            self.draw_text("Number-Nodes-Visited : {}".format(self.pacman.bfs.nodes_across), self.screen, [200,2], 
            START_TEXT_SIZE, WHITE, START_FONT)
            self.draw_text("ALGORITHM : BFS", self.screen, [WIDTH//2,2], 
            START_TEXT_SIZE, WHITE, START_FONT)
        else:
            self.draw_text("Number-Nodes-Visited : {}".format(self.pacman.astar.nodes_across), self.screen, [200,2], 
            START_TEXT_SIZE, WHITE, START_FONT)
